...
namespace AutocontrolService.Implementation.Config
{
    ...
    /// </summary>
    public class AutocontrolServiceConfiguration : ServiceConfiguration<AutocontrolServiceSettings>
    {
        /// <summary>
        ...
        {
            if (File.Exists(Path.Combine(basePath, ConfigurationConstants.AutocontrolServiceConfigFileName)))
            {
                configurationBuilder
                    .AddJsonFile(Path.Combine(basePath, ConfigurationConstants.AutocontrolServiceConfigFileName));

                SimpleLogging.Log(
                    "AutocontrolService-Configuration will be loaded from: " +
                    $"{Path.Combine(basePath, ConfigurationConstants.AutocontrolServiceConfigFileName)}",
                    SimpleLogEntryType.Information);
            }
            else
            {
                SimpleLogging.Log(
                    "AutocontrolService-Configuration file not found: " +
                    $"{Path.Combine(basePath, ConfigurationConstants.AutocontrolServiceConfigFileName)}",
        ...
        protected override void OnConfigurationFinished()
        {
            ...
            //    "Current AutocontrolService configuration:");
        }
        ...
        protected override void CreateCommandLineArguments(CommandLineParserBuilder<AutocontrolServiceSettings> parserBuilder)
        {
